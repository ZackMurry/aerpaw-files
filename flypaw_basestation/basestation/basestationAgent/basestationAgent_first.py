...
print("BEGIN: base station agent")
...
class FlyPawBasestationAgent(object):
    def __init__(self, ipaddr="192.168.139.2", port=20001, chunkSize=1024) :
        self.ipaddr = ipaddr
        self.port = port
        ...
        if processedPlan['STATUS'] == "UNINITIALIZED":
            ...
            print([x.Type for x in mission.missionObjectives])

        for i in mission.default_waypoints:
            print(f"WAYPOINT: {i}")
    ...
    def is_drone_between_points(self, dronePosition, p1, p2):
        #crossprod = (float(p2[1])-dronePosition.lat) * (float(p1[0])-dronePosition.lon) - (float(p2[0])-dronePosition.lon) * (float(p1[1])-dronePosition.lat)
        crossprod = (p2[1]-dronePosition.lat) * (p1[0]-dronePosition.lon) - (p2[0]-dronePosition.lon) * (p1[1]-dronePosition.lat)
        crossprod *= pow(10, 10)
        print(f"crossprod: {crossprod}")
        return abs(crossprod) < 5


    def is_drone_in_bounds(self, dronePosition):
        waypoints = self.missions[0].default_waypoints # each is[lon, lat, alt]
        for i in range(len(waypoints)-1):
            if waypoints[i][0] != waypoints[i+1][0] or waypoints[i][1] != waypoints[i+1][1]:
                if self.is_drone_between_points(dronePosition, waypoints[i], waypoints[i+1]):
                    print("BETWEEN!")
                    return True
        
        for i in range(len(waypoints)):
            if pow(10,8)*(pow(abs(dronePosition.lon - waypoints[i][0]), 2) + pow(abs(dronePosition.lat - waypoints[i][1]), 2)) < 1:
                print(f"CLOSE TO {i}")
                return True
    ...
    def basestationDispatch_SIM(self):#Simulates an unreliable connection. Doesn't respond if there is no connection
        ...
        while(True):
            ...
            serialClientMessage = bytesAddressPair[0]
            address = bytesAddressPair[1]
            print(f"CURRENT LOCATION ({str(self.droneSim.position.lon)}, {str(self.droneSim.position.lat)}, {str(self.droneSim.position.alt)})")
            if isinstance(self.droneSim.position.lon, float):
                self.is_drone_in_bounds(self.droneSim.position)
            try:
                clientMessage = pickle.loads(serialClientMessage)    
                print(clientMessage['type'])
                ...
                if msgType == "mission":
                    ...
                    for ms in self.missions:
                        if ms.STATUS == "PROCESSED" :
                            validMissions.append(ms)
                            print([x.Type for x in ms.missionObjectives])
                    print("Dispatcher-- mission check" + str(validMissions[0].missionObjectives))
                    msgFromServer['missions'] = validMissions
