...
def register_frame():
    ...
    frames_received[math.floor(time.time() - frame_graph_start)] += 1

def register_fake_frame():
    fake_frames[math.floor(time.time() - frame_graph_start)] += 1
    ...
    if msg:
      ...
      if msg.get_type() == 'ENCAPSULATED_DATA':
        ...
        if msg.seqnr <= last_seqnr:
          print(f"Warning: sequence numbers not received in order! Last: {last_seqnr}, Current: {msg.seqnr}")
          continue
...
def handle_video(data_conn):
    ...
    out = cv2.VideoWriter(f"output-{time_str}.mp4", fourcc, output_images_per_second, (1280,720))
    images_received = 0
    ...
    while not should_stop.is_set():
        ...
        if use_encryption:
            ...
            nonce = buffer[:16]
            tag = buffer[16:32]
            enc_session_key = buffer[32:32+256]
            ciphertext = buffer[32+256:]
            #print(f"nonce: {nonce}")
            #print(f"tag: {tag}")
            #print(f"enc_session_key: {enc_session_key}")
            #print(f"ciphertext: {ciphertext[0:30]}")
            cipher_rsa = PKCS1_OAEP.new(keyPair)
            session_key = cipher_rsa.decrypt(enc_session_key)
            cipher_aes = AES.new(session_key, AES.MODE_EAX, nonce)
            plaintext = bytearray(cipher_aes.decrypt_and_verify(ciphertext, tag))
            #print(f"plaintext: {plaintext[0:30]}")
            ...
            if use_signature and not is_signature_valid_dss(buffer, signature):
                  print('Invalid signature!')
                  register_fake_frame()
                  continue
        elif use_signature:
...
def handle_client(conn):
    ...
    time_str = datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    with open(f"frames_{time_str}.txt", "w") as file:
        for i in range(len(frames_received)):
            file.write(f"{i}, {frames_received[i]}\n")
    with open(f"fake_frames_{time_str}.txt", "w") as file:
        for i in range(len(fake_frames)):
            file.write(f"{i}, {fake_frames[i]}\n")
    #conn.port.close()
