...
class iperfInfo(object):
    def __init__(self, ipaddr="192.168.106.2", port=5201, protocol="tcp", priority=0, mbps=0, meanrtt=0):
...
class sendFrameInfo(object):
    def __init__(self, dataformat="jpgframes", ipaddr="192.168.106.2", port="8096", priority=1):
        self.dataformat = dataformat #jpgframes, ffmpeg, etc
...
class sendVideoInfo(object):
    def __init__(self, dataformat="jpgframes", ipaddr="192.168.106.2", port="23000", priority=1):
        self.dataformat = dataformat #jpgframes, ffmpeg, etc
        self.ipaddr = ipaddr #string ip address
        self.port = port #int port number 
        self.priority = priority #normalized float 0-1
...
class SpeculativeProduct(object):
    ...
    def Recommend(self):
        ...
        print([x.DecisionStack for x in self.Solutions])
        for sol in self.Solutions:
            if len(sol.DecisionStack) == 0:
                continue
            print(f"SOL: {sol.OptionNumber}")
        ...
        print(self.Solutions[bestSolution.OptionNumber].DecisionStack)
