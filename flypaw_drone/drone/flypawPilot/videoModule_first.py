...
injection_attack_start = 20
injection_attack_end = 30
...
def send_fake_image(conn):
  print(f"Sending fake frame")
  image = cv2.imread(f"/root/flypaw/drone/flypawPilot/data/tiny.jpg")
  ...
  b = ob
  # FAKE IMAGE, NOT REAL CODE
  if use_encryption:
    # RSA
    session_key = get_random_bytes(16) # 16 bytes
    cipher_rsa = PKCS1_OAEP.new(keyPair)
    enc_session_key = cipher_rsa.encrypt(session_key)
    print(f"len(enc_session_key): {len(enc_session_key)}")
    cipher_aes = AES.new(session_key, AES.MODE_EAX)
    ciphertext, tag = cipher_aes.encrypt_and_digest(ob)
    print(f"len(tag): {len(tag)}")
    ...
    b = bytearray(cipher_aes.nonce + tag + enc_session_key + b)
    #print(f"leb(b): {len(b)}")

  if use_signature:
    signature = bytearray([0]*56)
...
def send_image(conn, idx, t0):
  ...
  if use_encryption:
    ...
    #b = bytearray(cipher.nonce + tag + b)
    #b = bytearray(cipher.nonce + b)
    b = bytearray(nonce + tag + enc_session_key + ciphertext)
    #print(f"leb(b): {len(b)}")

  if use_signature:
    signature = create_signature_dss(ob)
    #print(f"signature: {signature}")
    #print(f"len(signature): {len(signature)}")
    b = bytearray(signature + b)
  ...
  while not should_stop.is_set():
    ...
    if msg.get_type() == 'DATA_TRANSMISSION_HANDSHAKE':
      ...
      if injection_attack:
          ...
          if dt > injection_attack_start and dt < injection_attack_end:
              for i in range(3):
                  send_fake_image(conn)
      idx = idx + 1
