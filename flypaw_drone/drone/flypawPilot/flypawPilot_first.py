...
class FlyPawPilot(StateMachine):
    def __init__(self):
        ...
        self.basestationIP = "192.168.139.2" #other side of the radio link
    ...
    def is_drone_between_points(self, dronePosition, p1, p2):
        crossprod = (p2.lat-dronePosition.lat) * (p1.lon-dronePosition.lon) - (p2.lon-dronePosition.lon) * (p1.lat-dronePosition.lat)
        crossprod *= pow(10, 10)
        print(f"crossprod: {crossprod}")
        return abs(crossprod) < 5


    def checkPhysicalCapture(self, drone: Drone):
        self.currentPosition = getCurrentPosition(drone)
        if self.PreviousTask == None or isinstance(self.PreviousTask.position, int):
            print("NO PREVIOUS TASK")
            if self.PreviousTask != None:
                print("ISINSTANCE")
            return
        if self.CurrentTask == None or isinstance(self.CurrentTask.position, int):
            print("NO CURRENT TASK")
            if self.CurrentTask != None:
                print("ISINSTANCE")
            return
        print("TASKS!")
        p1 = self.PreviousTask.position
        p2 = self.CurrentTask.position
        if pow(10,8)*(pow(abs(self.currentPosition.lon - p1.lon), 2) + pow(abs(self.currentPosition.lat - p1.lat), 2)) < 1:
            print("CLOSE TO PREVIOUS TASK")
            return
        if pow(10,8)*(pow(abs(self.currentPosition.lon - p2.lon), 2) + pow(abs(self.currentPosition.lat - p2.lat), 2)) < 1:
            print("CLOSE TO CURRENT TASK TASK")
            return
        if self.is_drone_between_points(self.currentPosition, p1, p2):
            print("BETWEEN POINTS")
            return

        print("PHYSICAL CAPTURE")
    ...
    async def waypoint_entry(self, drone: Drone):
        ...
        #self.checkPhysicalCapture(drone)
    ...
    async def completed(self, _ ):
        ...
        with open(f"/root/Results/{time_str}__{config_str}_DATA.txt", 'w') as experiment_log:
            ...
            for i in range(3):
                experiment_log.write(f"FPS {i+1}: {fps_results[i]}\n")
                experiment_log.write(f"Time (s) {i+1}: {time_results[i]}\n")
                experiment_log.write(f"BW (kb/s) {i+1}: {bw_results[i]}\n\n")
        sys.exit()
...
#            return 0
    ...
    def InterpretObjective(self, objective:MissionObjective):
        ...
        elif objectiveType == "IMAGE_LOCATION_QUICK":
            taskList.append(Task(objective.Waypoint,"FLIGHT",0,0,self.TaskIDGen.Get()))
    ...
    if serverReply is not None:
        ...
        if serverReply['uuid_received'] == str(x):
            ...
            if 'missions' in serverReply:
                ...
                print("missionObjectives: " + str([x.Type for x in missionObjectives]))
...
def udpClientMsg(msg, address, port, timeout_in_seconds):
    print(f"Messaging {address}:{port}")
    try:
