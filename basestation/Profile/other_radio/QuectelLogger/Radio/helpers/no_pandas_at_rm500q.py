...
if __name__ == '__main__':
    ...
    connected_string  = "========" ; spaces = '='.join([''] * (int(columns) - len(connected_string) - 10)) ; empty_print = spaces ; print(current_milli_time(), connected_string, empty_print, end="\n")
    ...
    with serial.threaded.ReaderThread(ser, RM500Q) as Quectel_module:
            ...
            if (len(lst_Var_Serving_cell_string) == 1):
                ...
                print(spaces2)
                #row_format0 = (len(Dict_Serving_cell_Line1.keys())) * "{:>15}"
                #print(row_format0.format(*Dict_Serving_cell_Line1.keys())) ; print(spaces1)
                #print(row_format0.format(*Dict_Serving_cell_Line1.values())) ; print(spaces2, end="\n")

                row_format1 = (len(Dict_Serving_cell_Line2.keys())) * "{:>15}"
                print(row_format1.format(*Dict_Serving_cell_Line2.keys())) ; print(spaces1)
                print(row_format1.format(*Dict_Serving_cell_Line2.values())) ; print(spaces2, end="\n")

                row_format2 = (len(Dict_Serving_cell_Line3.keys())) * "{:>20}"
                print(row_format2.format(*Dict_Serving_cell_Line3.keys())) ; print(spaces1)
                print(row_format2.format(*Dict_Serving_cell_Line3.values())) ; print(spaces2, end="\n\n")

                #print(tabulate(Dict_Serving_cell_Line2, headers='keys'))
                #print(tabulate(Dict_Serving_cell_Line2.items()))
            ...
            if (len(lst_Var_Serving_cell_string) == 1):
                Servingcell_LTE_params_Write = [current_milli_time()] ; Servingcell_LTE_params_Write = Servingcell_LTE_params_Write + Dict_Serving_cell_Line_LTE.values()
                Servingcell_LTE_Params_OrderedDict = OrderedDict(zip(Header_Servingcell_LTE, Servingcell_LTE_params_Write))
                with open('/root/Results/logs/{}/csv_kpi/Serving_cell_Params_LTE_only.csv'.format(str(args.time_dir)), 'a') as file_serving_lte:
                ...
                Servingcell_ENDC_params_Write = [current_milli_time()] ; Servingcell_ENDC_params_Write = Servingcell_ENDC_params_Write + Dict_Serving_cell_Line1.values()
                ...
                with open('/root/Results/logs/{}/csv_kpi/Serving_cell_Params_ENDC.csv'.format(str(args.time_dir)), 'a') as file_serving_endc:
            ...
            for i in range (len(lst_Var_Neighbour_cell_string)):
                ...
                print(row_format3.format(*Dict_Neighbour_cell_Line.keys())) ; print(spaces1)
                print(row_format4.format(*Dict_Neighbour_cell_Line.values())) ; print(spaces2, end="\n")
            print("\n")
            ...
            for i in range (len(lst_Var_Neighbour_cell_string)):
                ...
                Dict_Neighbour_cell_Line = []
                if((re.search(r"(?<=intra).*",lst_Var_Neighbour_cell_string[i]))):
                    Neighbour_cell_intra_params_Write = [current_milli_time()] ; Neighbour_cell_intra_params_Write_params_Write = Neighbour_cell_intra_params_Write + lst_Var_Neighbour_cell_string[i].split(",")
                    Dict_Neighbour_cell_Line = OrderedDict(zip(Header_Neighbourcell_intra, Neighbour_cell_intra_params_Write_params_Write))
                    with open('/root/Results/logs/{}/csv_kpi/Neighbourcell_Params_intra_.csv'.format(str(args.time_dir)), 'a') as file_neighbour_intra:
                ...
                if((re.search(r"(?<=inter).*",lst_Var_Neighbour_cell_string[i]))):
                    Neighbour_cell_inter_params_Write = [current_milli_time()] ; Neighbour_cell_inter_params_Write_params_Write = Neighbour_cell_inter_params_Write + lst_Var_Neighbour_cell_string[i].split(",")
                    Dict_Neighbour_cell_Line = OrderedDict(zip(Header_Neighbourcell_inter, Neighbour_cell_inter_params_Write_params_Write))
                    with open('/root/Results/logs/{}/csv_kpi/Neighbourcell_Params_inter_.csv'.format(str(args.time_dir)), 'a') as file_neighbour_inter:
            ...
            print(row_format5.format(*Dict_QNWCFG_LTE_CSI_Line.keys())) ; print(spaces1)
            print(row_format5.format(*Dict_QNWCFG_LTE_CSI_Line.values())) ; print(spaces2, end="\n")
            print("\n")
            #Write to CSV basic & other params
            Basic_Param_and_Other_params_Write = [current_milli_time()]
            Basic_Param_and_Other_params_Write = Basic_Param_and_Other_params_Write + Dict_QNWCFG_LTE_CSI_Line.values() + Dict_QNWCFG_NR_Line.values()
            ...
            with open('/root/Results/logs/{}/csv_kpi/Basic_and_Other_Params.csv'.format(str(args.time_dir)), 'a') as file_basic:
    ...
    empty_print = spaces ; print(current_milli_time(), "\n", disconnected_string, empty_print)
