...
CONFIG_PATH=/root/Profiles/ProfileScripts/Radio/Helpers/Handover/rrHO5.conf
...
if [ $LAUNCH_MODE == "TESTBED" ]; then
    bash -c "printf 't\n' | srsenb --rf.tx_gain=$tx_gain --rf.rx_gain=$rx_gain --rf.dl_earfcn=$earfcn --enb.n_prb=$n_prb --enb_files.rr_config=$CONFIG_PATH --enb.mme_addr=$mme_addr --enb.gtp_bind_addr=$gtp_s1c_bind_addr --enb.s1c_bind_addr=$gtp_s1c_bind_addr --enb.enb_id=0x19F"
elif [ $LAUNCH_MODE == "EMULATION" ]; then
    bash -c "printf 't\n' | srsenb --rf.srate=11.52e6 --rf.tx_gain=$tx_gain --rf.rx_gain=$rx_gain --rf.dl_earfcn=$earfcn --enb.n_prb=$n_prb --enb_files.rr_config=$CONFIG_PATH --enb.mme_addr=$mme_addr --enb.gtp_bind_addr=$gtp_s1c_bind_addr --enb.s1c_bind_addr=$gtp_s1c_bind_addr --enb.enb_id=0x19F"
fi
